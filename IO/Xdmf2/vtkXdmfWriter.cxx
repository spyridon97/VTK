...
struct vtkXW2NodeHelp
{
  ...
  XdmfXmlNode node;
  bool staticFlag;
  vtkXW2NodeHelp(xdmf2::XdmfDOM* d, XdmfXmlNode n, bool f)
    : DOM(d)
    , node(n)
    , staticFlag(f)
  {
...
{
public:
  vtkXdmfWriterDomainMemoryHandler() { domain = new XdmfDomain(); }
...
struct vtkXdmfWriterInternal
{
  ...
  public:
    ...
    {
      return this->VTKType < ct.VTKType ||
        (this->VTKType == ct.VTKType && this->NumPoints < ct.NumPoints);
    }
    bool operator==(const CellType& ct) const
    {
  ...
  vtkPointSet* t, vtkXdmfWriterInternal::MapOfCellTypes& vec)
{
  ...
  for (cc = 0; cc < t->GetNumberOfCells(); cc++)
  {
    ...
    if (it == vec.end())
    {
      ...
      it = vec
             .insert(
               vtkXdmfWriterInternal::MapOfCellTypes::value_type(ct, vtkSmartPointer<vtkIdList>(l)))
             .first;
      l->Delete();
    }
    ...
    if (dsa)
    {
      ...
      switch (attributeType)
      {
        ...
        case vtkDataSetAttributes::GLOBALIDS:
          ...
          break;
        case vtkDataSetAttributes::TENSORS: // TODO: vtk tensors are 9 component, xdmf tensors are
                                            // 6?
...
//------------------------------------------------------------------------------
void vtkXdmfWriter::ConvertVToXArray(vtkDataArray* vda, XdmfArray* xda, vtkIdType rank,
  vtkIdType* dims, int allocStrategy, const char* heavyprefix)
{
  ...
  switch (vda->GetDataType())
  {
    ...
    case VTK_ID_TYPE:
      xda->SetNumberType(
        (VTK_SIZEOF_ID_TYPE == sizeof(XDMF_64_INT) ? XDMF_INT64_TYPE : XDMF_INT32_TYPE));
      break;
    case VTK_LONG:
