  ...
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* vtkNotUsed(outputVector))
{
  ...
  else
  {
    ...
    while (attempts < 2)
    {
      ...
      vtkDebugMacro(<< this->NumberOfNodes << " " << this->NumberOfCells << " "
                    << this->NumberOfNodeFields << " " << this->NumberOfCellFields << " "
                    << this->NumberOfFields << " " << this->NlistNodes << endl);
      ...
      vtkDebugMacro(<< "TFL = " << trueFileLength << "\tCFL = " << calculatedFileLength << endl);
    ...
    long offset =
      base_offset + 16 * this->NumberOfCells + 4 * this->NlistNodes + 3 * 4 * this->NumberOfNodes;
    ...
    if (this->NumberOfNodeFields)
    {
      ...
      for (i = 0; i < this->NumberOfNodeComponents; i++)
      {
        ...
        vtkDebugMacro(<< i + 1 << " :found ND label = " << label << " [" << ncomp_list[i] << "]"
                      << endl);
    ...
    if (this->NumberOfCellFields)
    {
      ...
      {
        this->GetLabel(buffer1, i, label);
        vtkDebugMacro(<< i + 1 << " :found CD label = " << label << " [" << ncomp_list[i] << "]"
                      << endl);
  ...
  {
    vtkDebugMacro(<< endl
                  << this->PointDataArraySelection->GetArrayName(i) << endl
  ...
  {
    vtkDebugMacro(<< endl
                  << this->CellDataArraySelection->GetArrayName(i) << endl
  ...
  vtkIntArray* materials, int* types, vtkIdTypeArray* listcells)
{
  ...
  for (i = 0; i < this->NumberOfCells; i++)
  {
    *mat++ = ctype[4 * i + 1];
    switch (ctype[4 * i + 3])
    {
      case vtkAVSucdReader::PT:
        types[i] = VTK_VERTEX;
        break;
      case vtkAVSucdReader::LINE:
        types[i] = VTK_LINE;
        break;
      case vtkAVSucdReader::TRI:
        types[i] = VTK_TRIANGLE;
        break;
      case vtkAVSucdReader::QUAD:
        types[i] = VTK_QUAD;
        break;
      case vtkAVSucdReader::TET:
        types[i] = VTK_TETRA;
        break;
      case vtkAVSucdReader::PYR:
        types[i] = VTK_PYRAMID;
        break;
      case vtkAVSucdReader::PRISM:
        types[i] = VTK_WEDGE;
        break;
      case vtkAVSucdReader::HEX:
