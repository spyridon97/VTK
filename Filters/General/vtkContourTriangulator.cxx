...
void vtkContourTriangulator::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "TriangulationError: " << this->TriangulationError << "\n";
...
void vtkCCSMakePolysFromLines(vtkPolyData* data, vtkIdType firstLine, vtkIdType endLine,
  bool oriented, std::vector<vtkCCSPoly>& newPolys, std::vector<size_t>& incompletePolys);
...
int vtkCCSSplitAtPinchPoints(std::vector<vtkCCSPoly>& polys, vtkPoints* points,
...
int vtkCCSVectorProgression(const double p[3], const double p1[3], const double p2[3],
  const double p3[3], const double normal[3]);
...
void vtkCCSFindTrueEdges(std::vector<vtkCCSPoly>& newPolys, vtkPoints* points,
...
void vtkCCSReversePoly(vtkCCSPoly& poly, vtkCCSPolyEdges& edges, vtkCCSCellArray& originalEdges);
...
// each triangle.
void vtkCCSInsertTriangle(vtkCellArray* polys, const vtkCCSPoly& poly, const size_t trids[3],
...
void vtkCCSMakeHoleyPolys(std::vector<vtkCCSPoly>& polys, vtkPoints* points,
  std::vector<vtkCCSPolyGroup>& polyGroups, std::vector<vtkCCSPolyEdges>& polyEdges,
  vtkCCSCellArray& originalEdges, const double normal[3], bool oriented);
...
int vtkCCSCutHoleyPolys(std::vector<vtkCCSPoly>& polys, vtkPoints* points,
  std::vector<vtkCCSPolyGroup>& polyGroups, std::vector<vtkCCSPolyEdges>& polyEdges,
...
double vtkCCSTriangleQuality(
  const double p0[3], const double p1[3], const double p2[3], const double normal[3]);
  ...
  const double p0[3], const double p1[3], const double p2[3], const double normal[3])
{
  ...
  double area2 = (u[1] * v[2] - u[2] * v[1]) * normal[0] + (u[2] * v[0] - u[0] * v[2]) * normal[1] +
    (u[0] * v[1] - u[1] * v[0]) * normal[2];

  double perim = sqrt(u[0] * u[0] + u[1] * u[1] + u[2] * u[2]) +
    sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]) + sqrt(w[0] * w[0] + w[1] * w[1] + w[2] * w[2]);
  ...
  while (remainingLines > 0)
  {
    ...
    while (!completePoly && !noLinesMatch && remainingLines > 0)
    {
      ...
      for (int endIdx = 0; endIdx < 2; endIdx++)
      {
        ...
        {
          lineId = cells[icell];
          if (lineId >= firstLine && lineId < endLine && !usedLines.get(lineId - firstLine))
          {
            data->GetCellPoints(lineId, npts, pts);
            lineEndPts[0] = pts[0];
...
void vtkCCSMakeHoleyPolys(std::vector<vtkCCSPoly>& newPolys, vtkPoints* points,
  std::vector<vtkCCSPolyGroup>& polyGroups, std::vector<vtkCCSPolyEdges>& polyEdges,
  vtkCCSCellArray& originalEdges, const double normal[3], bool oriented)
{
  ...
  for (size_t i = 0; i < numNewPolys; i++)
  {
    ...
    for (size_t j = 0; j < numNewPolys; j++)
    {
      if (j != i && newPolys[j].size() >= 3)
      {
        ...
        {
          if (vtkCCSPolyInPoly(newPolys[i], newPolys[j], points, normal, pp, bounds, tol2))
          {
            // Add to group
...
int vtkCCSCheckCut(const std::vector<vtkCCSPoly>& polys, vtkPoints* points, const double normal[3],
...
int vtkCCSFindCuts(const std::vector<vtkCCSPoly>& polys, const vtkCCSPolyGroup& polyGroup,
  ...
  for (cutId = 0; cutId < 2; cutId++)
  {
    ...
    for (size_t i = 0; i < count && !foundCut; i++)
    {
      ...
      for (size_t lid = 0; lid < cutlist.size(); lid++)
      {
        ...
        if (vtkCCSCheckCut(polys, points, normal, polyGroup, outerPolyId, innerPolyId,
              static_cast<vtkIdType>(k), static_cast<vtkIdType>(j)))
        {
          cuts[cutId][0] = k;
          cuts[cutId][1] = j;
          foundCut = 1;
    ...
    if (polyGroup.size() > 1)
    {
      ...
      for (int exhaustive = 0; exhaustive < 2 && !madeCut; exhaustive++)
      {
        for (size_t j = 1; j < polyGroup.size(); j++)
        {
          ...
          size_t cuts[2][2];
          if (vtkCCSFindCuts(
                polys, polyGroup, outerPolyId, innerPolyId, points, normal, cuts, exhaustive))
          {
  ...
  vtkIdType numLines, vtkCellArray* polys, const double normal[3])
{
  ...
  vtkCCSMakePolysFromLines(
    data, firstLine, firstLine + numLines, oriented, newPolys, incompletePolys);
  ...
  vtkCCSMakeHoleyPolys(newPolys, points, polyGroups, polyEdges, originalEdges, normal, oriented);
  ...
  vtkCCSSplitAtPinchPoints(newPolys, points, polyGroups, polyEdges, normal, oriented);
  ...
  for (size_t polyId = 0; polyId < polyGroups.size(); polyId++)
  {
    ...
    if (!vtkCCSTriangulate(
          newPolys[polyId], points, polyEdges[polyId], originalEdges, polys, normal))
    {
      triangulationFailure = 1;
