  ...
  vtkDataObjectCollection* inMetaColl, vtkMultiBlockDataSet* outMeta)
{
  ...
  while ((inMetaDO = inMetaColl->GetNextDataObject(it)))
  {
    ...
    for (int r = 0; r < nRow; ++r)
    {
      ...
      if (inCov->GetValueByName(r, VTK_MULTICORRELATIVE_KEYCOLUMN1) !=
          outCov->GetValueByName(r, VTK_MULTICORRELATIVE_KEYCOLUMN1) ||
      ...
      else if (inCov->GetValueByName(r, VTK_MULTICORRELATIVE_KEYCOLUMN2).ToString().empty())
      {
        ...
        outCov->SetValueByName(r, VTK_MULTICORRELATIVE_ENTRIESCOL,
...
void vtkMultiCorrelativeStatistics::Derive(vtkMultiBlockDataSet* outMeta)
{
  ...
  if (!outMeta || !(sparseCov = vtkTable::SafeDownCast(outMeta->GetBlock(0))) ||
    ...
    !(col2 = vtkArrayDownCast<vtkStringArray>(
        sparseCov->GetColumnByName(VTK_MULTICORRELATIVE_KEYCOLUMN2))) ||
    !(col3 = vtkArrayDownCast<vtkDoubleArray>(
        sparseCov->GetColumnByName(VTK_MULTICORRELATIVE_ENTRIESCOL))))
  {
    return;
  }
  ...
  for (; i < ncol3; ++i)
  {
    std::pair<vtkIdType, vtkIdType> idxs(
       ...
       ++reqIt, ++b)
  {
    ...
    vtkIdType j = 0;
    for (std::vector<vtkDoubleArray*>::iterator arrIt = covCols.begin(); arrIt != covCols.end();
         ++arrIt, ++j)
    {
  ...
  vtkTable* inData, vtkMultiBlockDataSet* inMeta, vtkTable* outData)
{
  ...
  for (int req = 1; req < nb; ++req)
  {
    ...
    vtkMultiCorrelativeAssessFunctor* mcfunc =
      static_cast<vtkMultiCorrelativeAssessFunctor*>(dfunc);
    if (!mcfunc)
    {
...
void vtkMultiCorrelativeStatistics::ComputeMedian(vtkTable* inData, vtkTable* outData)
{
  ...
  orderStats->SetInputData(vtkStatisticsAlgorithm::INPUT_DATA, inOrderStats);
  ...
  vtkTable* inData, vtkTable* reqModel, bool cholesky)
{
  ...
  if (!avgs)
  {
    vtkGenericWarningMacro(
      "Multicorrelative request without a \"" VTK_MULTICORRELATIVE_AVERAGECOL "\" column");
    return false;
  }
  vtkStringArray* name =
    vtkArrayDownCast<vtkStringArray>(reqModel->GetColumnByName(VTK_MULTICORRELATIVE_COLUMNAMES));
  if (!name)
  {
    vtkGenericWarningMacro(
      "Multicorrelative request without a \"" VTK_MULTICORRELATIVE_COLUMNAMES "\" column");
    return false;
  }
