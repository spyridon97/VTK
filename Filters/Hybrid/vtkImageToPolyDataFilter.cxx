...
void vtkImageToPolyDataFilter::BuildPolygons(vtkUnsignedCharArray* vtkNotUsed(pointDescr),
  vtkPolyData* edges, int numPolys, vtkUnsignedCharArray* polyColors)
{
  vtkPoints* points = edges->GetPoints();
        ...
        { // build loop
          ...
          while (true)
          {
            ...
            for (k = 0; k < ncells2; k++)
            {
              ...
              if (cells2[k] != edgeId && (polyId2[0] == polyId[j] || polyId2[1] == polyId[j]))
              {
                p1 = p2;
                edgeId = cells2[k];
                break;
...
//
void vtkImageToPolyDataFilter::DecimateEdges(
  vtkPolyData* edges, vtkUnsignedCharArray* pointDescr, double tol2)
{
  vtkPoints* points = edges->GetPoints();
