...
static int vtkDataObjectGeneratorGetNextToken(char** str)
{
  ...
  while (len && *str)
  {
    for (int i = 0; i < NUMTOKENS; i++)
    {
      ...
      if (len >= l && !strncmp(*str, vtkDataObjectGeneratorTokenStrings[i], l))
      {
        *str += l;
        return i;
      }
  ...
  vtkInformation*, vtkInformationVector**, vtkInformationVector* outV)
{
  ...
  {
    this->Rank = outInfo->Get(vtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
  }
  this->Processors = 1;
  if (outInfo->Has(vtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES()))
  {
    this->Processors = outInfo->Get(vtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES());
  }
  ...
  vtkInternalStructureCache* structure, int level, int stripe)
{
  ...
  switch (t)
  {
    ...
    case MBS:
    {
      vtkMultiBlockDataSet* mbo = vtkMultiBlockDataSet::SafeDownCast(outData);
