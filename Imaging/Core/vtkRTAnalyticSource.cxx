  ...
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  ...
  if (this->WholeExtent[0] > this->WholeExtent[1] || this->WholeExtent[2] > this->WholeExtent[3] ||
    this->WholeExtent[4] > this->WholeExtent[5])
  {
    vtkErrorMacro("Invalid WholeExtent: " << this->WholeExtent[0] << ", " << this->WholeExtent[1]
                                          << ", " << this->WholeExtent[2] << ", "
  ...
  outInfo->Set(vtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), tmpExt, 6);

  outInfo->Set(vtkDataObject::ORIGIN(), 0.0, 0.0, 0.0);
  outInfo->Set(
    vtkDataObject::SPACING(), this->SubsampleRate, this->SubsampleRate, this->SubsampleRate);
...
void vtkRTAnalyticSource::ExecuteDataWithInformation(
  vtkDataObject* vtkNotUsed(output), vtkInformation* outInfo)
{
  ...
  */
  ...
  data->SetSpacing(this->SubsampleRate, this->SubsampleRate, this->SubsampleRate);
  ...
  for (idxZ = 0; idxZ <= maxZ; idxZ++)
  {
    ...
    for (idxY = 0; !this->AbortExecute && idxY <= maxY; idxY++)
    {
      ...
      for (idxX = 0; idxX <= maxX; idxX++)
      {
        ...
        *outPtr = this->Maximum * exp(-sum * temp2) + xfactor /*this->XMag*sin(this->XFreq*x)*/
