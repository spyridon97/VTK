...
template <class T>
void vtkImageVariance3DExecute(vtkImageVariance3D* self, vtkImageData* mask, vtkImageData* inData,
            ...
            for (hoodIdx1 = hoodMin1; hoodIdx1 <= hoodMax1; ++hoodIdx1)
            {
              ...
              for (hoodIdx0 = hoodMin0; hoodIdx0 <= hoodMax0; ++hoodIdx0)
              {
                  ...
                  outIdx2 + hoodIdx2 >= inImageMin2 && outIdx2 + hoodIdx2 <= inImageMax2)
                {
                  if (*maskPtr0)
                  {
                    diff = static_cast<float>(*hoodPtr0) - static_cast<float>(*inPtr0);
                    sum += diff * diff;
                    ++icount;
                  }
                }
...
void vtkImageVariance3D::ThreadedRequestData(vtkInformation* vtkNotUsed(request),
  ...
  vtkImageData*** inData, vtkImageData** outData, int outExt[6], int id)
{
  ...
  {
    vtkTemplateMacro(vtkImageVariance3DExecute(this, mask, inData[0][0],
      static_cast<VTK_TT*>(inPtr), outData[0], outExt, static_cast<float*>(outPtr), id, inInfo));
    default:
